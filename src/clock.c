...
void createCombinational(CombinationalState *C) {
    // WB
    C->WB_DATA = pipeline.MEM_WEB.ctrl.memToReg ? pipeline.MEM_WEB.ulaOut : pipeline.MEM_WEB.MEM;

    // MEM
    C->MEM_MEM = memData.data[pipeline.EX_MEM.ulaOut];
    C->MEM_ulaOut = pipeline.EX_MEM.ulaOut;
...
void clock() {
    ...
    pipeline.MEM_WEB.ctrl = pipeline.EX_MEM.ctrl;
...
//     pipeline->EX_MEM.RD = pipeline->ID.ctrl.regDst ? pipeline->ID.RS : pipeline->ID.RT;
//     printf("rd mux is %d and value %d\n ", pipeline->ID.ctrl.regDst,
//            (pipeline->ID.ctrl.regDst ? pipeline->ID.RS : pipeline->ID.RT));
